...
def test_native_v2_distribution_has_no_legacy_converter():
    import rsl_rl.utils
    from rsl_rl.utils import checkpoint

    for path in ("tools/convert_legacy_checkpoint.py", "tests/test_checkpoint_legacy_converter.py"):
        assert not (ROOT / path).exists()
    for module in (rsl_rl.utils, checkpoint):
        assert not hasattr(module, "convert_legacy_checkpoint_once")
    production = []
    for relative in ("training/rsl_rl/rsl_rl", "training/legged_gym/legged_gym",
                     "sea_nav_current_isaaclab_full_method", "tools"):
        production.extend(path for path in (ROOT / relative).rglob("*.py")
                          if "tests" not in path.relative_to(ROOT).parts)
    checked_loads = []
    forbidden_flags = {"--_sandbox-worker", "--_validate", "--trust-legacy"}
    for path in production:
        tree = ast.parse(path.read_text())
        torch_names = {alias.asname or alias.name for node in ast.walk(tree)
                       if isinstance(node, ast.Import) for alias in node.names if alias.name == "torch"}
        load_names = {alias.asname or alias.name for node in ast.walk(tree)
                      if isinstance(node, ast.ImportFrom) and node.module == "torch"
                      for alias in node.names if alias.name == "load"}
        for node in ast.walk(tree):
            if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                assert node.name != "convert_legacy_checkpoint_once", str(path)
            if isinstance(node, ast.Constant) and isinstance(node.value, str):
                assert node.value not in forbidden_flags, str(path)
            if not isinstance(node, ast.Call):
                continue
            function = node.func
            is_load = (isinstance(function, ast.Attribute) and function.attr == "load"
                       and isinstance(function.value, ast.Name) and function.value.id in torch_names)
            is_load = is_load or (isinstance(function, ast.Name) and function.id in load_names)
            if is_load:
                options = [item.value for item in node.keywords if item.arg == "weights_only"]
                assert len(options) == 1 and isinstance(options[0], ast.Constant) and options[0].value is True, str(path)
                checked_loads.append(path.relative_to(ROOT))
    assert checked_loads
    for relative in ("README.md", "sea_nav_current_isaaclab_full_method/README.md"):
        current_usage = (ROOT / relative).read_text()
        assert not any(text in current_usage for text in (
            "convert_legacy_checkpoint.py", "conversion-receipt.json", "operator converter"))


@pytest.mark.parametrize("old_flag", [
    ["--checkpoint", "1"], ["--checkpoint=1"],
    ["--init-checkpoint", "legacy.pt"], ["--init-checkpoint=legacy.pt"],
    ["--resume"], ["--resume=true"],
    ["--load_run", "latest"], ["--load_run=latest"],
    ["--load-run", "latest"], ["--load-run=latest"],
])
def test_native_v2_preflight_explicitly_rejects_old_checkpoint_flags(old_flag, tmp_path):
    from rsl_rl.runtime_preflight import preflight
    with pytest.raises(ValueError, match="legacy raw/numeric checkpoint flags are unsupported"):
        preflight(cli(tmp_path) + old_flag, runtime_stack="isaac_gym_preview4", repo_root=ROOT)
