...
|---|---|---|---|
| Artifact / version identity | SEA primary/remote `test@169aa1f7448ae3900a0dbcd3eb99595177d6003e`; accepted differential-drive candidate `2cd810569008fa923bda088f0f0988292e0c809c`; DashGo primary/remote `test@53bd0f0ab0100e295aed1f726c2c3c9f4ffb3b06` | 2026-09-08 branch readback plus independent source/CPU and artifact reviews | differential-drive candidate is accepted for D0 but not yet integrated into SEA `test`; no DashGo adapter source has landed |
| Active evidence instance | SEA Tasks1–7 integrated evidence; differential-drive source/CPU review (`284 passed` plus combined `626 passed`) and final artifact review (`284` frozen source, `284` sdist, `281` installed wheel); DashGo D1 preflight | exact OIDs, committed review receipts, fixed-version API sources, physical artifact hashes | artifact evidence is CPU/package-only; simulator and hardware are not implied |
...
| Unpublished/unverified work | ordered integration of `bf409c65→70f2304→2cd8105` into SEA `test`; all planned DashGo adapter code | detached candidate and current coordination state | publish each accepted slice only after integration tests, committed-tree scan, push, and remote readback |
...
- DashGo's pinned Isaac Lab v2.0.2 `RslRlVecEnvWrapper` returns tuple tensors, while pinned RSL-RL v3.0.1 expects TensorDict observations and `obs_groups`; the current training entrypoint cannot initialize that runner without an explicit compatibility adapter.
- RSL-RL v3.0.1 storage does not retain next observations, so `Lreg`/smoothness cannot be implemented correctly without custom storage and done-masked current/next minibatches.
- DashGo's current 246-D policy input is term-major `72x3 lidar + 3x3 waypoint + 3x3 goal + 1x3 linear velocity + 1x3 yaw rate + 2x3 last action`; CBF safety inputs must be a separate metric group with angles, validity, and age rather than reverse-engineered normalized policy features.
- Formal evaluation currently reads auto-reset post-terminal state; terminal metrics need a pre-reset snapshot or terminal observation contract, and replay must be disabled for formal evaluation.
- Task7 `92ab65d` is not transactional: a strict model-load or later optimizer incompatibility can leave model parameters changed after failure. Its Adam validation accepts target-incompatible moments/parameter groups that fail on the next update, and a hash-valid checkpoint can overwrite config-derived CBF ray geometry while the receipt still claims the configured FOV. These are independently reproduced P2 defects, not hypothetical risks or simulator-only blockers.
- The two adapter trainers currently label the manifest file size as `checkpoint_bytes`; the payload size must come from the validated manifest `byte_size` field or be named separately. This is a P3 evidence-accounting defect.
- Task7 fix1 `3b6e97d` closes the original four findings but introduced a load-time zero-gradient `optimizer.step()` probe. It both rejects a valid `betas=(0,0), eps=0` Adam state that survives a real next update and triggers global optimizer hooks/RNG. Failed post-load hooks also leave parameter gradients and RNG changed. Both are independently reproduced P2 transaction defects; fix2 must use pure validation and restore gradients/RNG on failure.
- Differential-drive candidate `bf409c65` passes its independent 155-case math/package suite but omits the approved MIT grant/metadata, cannot encode the DashGo wheel radius/track width/linear and angular acceleration contract, has no versioned raw metric safety-observation ABI, and does not retain or serialize the complete platform/observation identity needed to detect a wrong-runtime configuration. These four independent P2 findings block integration.
- The pre-wheel-projection CBF residual is not an executed-command safety receipt. A bounded acceleration projection can turn a positive biased residual into a negative applied residual, so the consumer/core boundary must recompute post-projection diagnostics and keep the two stages separately named.
- Fixed differential-drive candidate `2cd8105` closes the four original package/platform/safety-observation/identity findings and the post-projection receipt gap. Independent review found no P0-P3 issue; artifact verification accepts only the D0 source/CPU/package scope and explicitly excludes CUDA, simulator, ROS 2 runtime, plant dynamics, real-time behavior, and hardware.
...
- Original core helper's compact completed summary reports a same-inode concurrent-write digest/loaded-data mismatch candidate and synthetic fstat cleanup observation. Detailed handoff also hit the service restriction, so controller has not reproduced or independently certified it. Existing checkpoint writer was alerted to inspect immutable-read/cleanup correctness; the partial evidence and missing full commands are explicit in task-7-core-functional-review.md.

- Subsequent sole-writer report independently reproduced the same-inode problem using only a plain Tensor checkpoint (RED1 in.82s) and implemented a sealed memfd private snapshot for safe loader and operator approved-input bytes; unsupported sealing is blocked. Its related53-test run passed14.75s, with fstat/fdopen cleanup fixed. This is moving-worker implementation evidence, superseding candidate-only status as a reported reproduction but not supplying controller/fixed-commit acceptance. Full report/independent verification remain required.

- New fixedc7b9aa3 controller ordinary follow-through independently passes28 tests in2.55s, including real102/adaptive continuation and the three plain-Tensor sealed-snapshot regressions. This supplies GREEN for the specific same-inode correction, separate from the older candidate/worker observations; full core/converter review remains incomplete. Exact selection/identity and exclusions are in the functional-review addendum.
