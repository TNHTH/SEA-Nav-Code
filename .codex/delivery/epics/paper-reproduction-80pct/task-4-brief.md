...
Shared-core import compatibility: if the adapter layer reuses/inherits the packaged core (allowed dependency direction), correct the three existing bootstrap blocks in `full_method_runtime_smoke.py`, `train_full_method_ppo.py`, and `train_full_method_acsi_replay_ppo.py` now. They currently import `adapters.cbf_shield` before choosing bundled `rsl_rl`, then clear its modules; that would introduce import failure or split class/module identity. Scope is narrowly extended to derive `sea_root` from `adapter_root.parent`, select the bundled package before the new dependent adapter import, and remove the now-duplicated later bootstrap. Preserve AppLauncher ordering; no simulator imports in CPU tests. Add a fresh-process real-package bootstrap regression plus AST call-order checks without starting the app or inserting fake simulator modules. Full CLI/runtime profile/manifest wiring remains Task 6. Do not defer this newly introduced import-order regression as a simulator blocker.
