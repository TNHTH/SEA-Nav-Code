...
- **Status:** active fix round. The old `2387cf0→c7b9aa3→774027d` chain is retained only under an explicitly unaccepted checkpoint tag. Independent review rejected `92ab65d` for three reproducible P2 loading defects and one P3 byte-accounting defect; a single writer is repairing that candidate before a fresh independent rereview.
...
|---|---|---|---|---|---|
| `work/SEA-Nav-Code` | `test@b94aede1fe3135e0a80eae1ea69021efee37d9fd` plus current coordination diff | controller | plan/coordination now; each integrated batch serially | approved spec | active, Tasks1–6 integrated; remote read back at same commit |
...
| `work/SEA-Nav-Code-batch7-v2` | detached `3b6e97d0004eac6ff3f04be4e7b56acb693db414`; BASE candidate `92ab65d`, project BASE `399ce2b` | `/root/review_batch7_callers` sole fix2 writer | Task7 application/checkpoint tests and local fix ledgers only; two worker registration diffs retained | fix1 rereview FAIL: no executable optimizer probe; restore gradients/RNG on failure | fix2 active; unreviewed fix1 tag retained, not integrated |
| `work/SEA-Nav-Code-diffdrive-core` | detached `bf409c65ac2d6dc43bfacfd8f99c31cfe5537eb9`; BASE `399ce2b08eac40865fd6496d19324f73a3e6cc7e` | `/root/dashgo_adapter_audit/diffdrive_core_fix` | only `packages/sea_nav_core/**` and its existing focused tests/docs; retain two local registration diffs outside the functional commit | independent FAIL in `diffdrive-core-review.md`; accepted user contract and DashGo provenance | fix round active; no overlap with Task 7, not integrated or published |
