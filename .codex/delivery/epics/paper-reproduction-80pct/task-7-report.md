...
## Fix1 independent rereview verdict — 2026-09-08

Fix1 `3b6e97d0004eac6ff3f04be4e7b56acb693db414` closes the original unexpected-key, Adam-shape/group, CBF geometry and payload-size findings, but the independent rereview is still **FAIL** with two P2 transaction defects. The reviewer executed 213 focused passes with 6 simulator/expression deselections in 38.97 seconds, then separately ran the two payload-byte tests (2 passed in 1.53 seconds). No source or ref was changed.

- `checkpoint.py` performs a zero-gradient `optimizer.step()` on a disposable optimizer as a compatibility probe. This rejects a legal Adam state with `betas=(0,0), eps=0` as nonfinite even though the same state loaded natively into the real runner completes the next `learn(1)` with finite parameters. The probe also invokes official global optimizer step hooks and can advance RNG. Loading must use pure structural/target-state validation and must not execute an optimizer update.
- The exception rollback restores model tensors, optimizer state/groups/defaults, learning rate and iteration, but a post-load hook can change an existing parameter gradient and consume RNG before raising. Independent model and optimizer hook probes left gradients changed (`1→10` and `1→3`) and advanced RNG after failure. Transaction rollback must restore each parameter's gradient/`None` state and CPU plus initialized CUDA RNG states. This does not promise reversal of arbitrary external hook side effects.

The fix1 commit remains durably and accurately labeled by the remote annotated tag `checkpoint/task7-fix1-unreviewed-20260908-3b6e97d`; its peeled object is exactly `3b6e97d`. Fix2 is active in the same detached worktree. No named branch has accepted fix1.
