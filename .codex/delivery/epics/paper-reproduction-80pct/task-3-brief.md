...
Inherited gate correction: `sea_nav_current_isaaclab_full_method/tests/gate_a_static_contract.py::test_actor_noise_std_and_post_sample_projection` currently requires the removed local regression. Extend Task 3 ownership to replace this case and its direct-run registration with the correct mean-stage Normal sampling contract, retaining the explicit noise-std check. Use real actor/distribution behavior (seeded exact sample and likelihood), not merely deleting/skipping the test or preserving its fake-distribution projection expectation. Keep unrelated gate cases unchanged.
