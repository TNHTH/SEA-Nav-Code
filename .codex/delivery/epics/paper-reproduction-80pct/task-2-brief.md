...
Replay application projection also declares the compact `new_replay_episode_v1` policy, with explicit `replay_reset_reconstruction_v1` delta at its Task 5 activation boundary. Task 2 must support declared additional named repairs without silently inserting them; `ppo_state_identity_repair` is mandatory, not the only possible delta. Record inactive/future consumer contracts honestly until their implementation task lands.
