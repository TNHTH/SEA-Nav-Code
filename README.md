...
Loading requires the actual runtime's explicit `weights_only=True` support plus a successful v2 round trip and kernel-sealed file snapshots. An older Torch lacking this API is blocked; the CPU Torch result cannot authorize simulator resume. Historical raw `.pt`, numeric `--checkpoint`, `--resume`, and automatic latest-run selection are rejected. Legacy recovery is available only through the separately invoked OS-sandboxed operator tool `tools/convert_legacy_checkpoint.py`, with a preapproved SHA-256, an empty output directory, explicit producer/config hashes and independent safe output validation. Its acceptance receipt is `conversion-receipt.json`; failed output remains untrusted. It is never a training flag.
