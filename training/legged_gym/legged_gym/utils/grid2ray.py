...
def batch_ray_cast_torch_variable_origins(
    grid_batch,
    base_row,
    base_col,
    angles,
    *,
    ...
    """
    Batched ray casting where each environment can have its own base point and
    ray angles. Returns distances with shape (num_envs, num_rays).
    """
    device = grid_batch.device
    num_envs, n_x, n_y = grid_batch.shape
    base_row = torch.as_tensor(base_row, device=device)
    base_col = torch.as_tensor(base_col, device=device)
    if base_row.numel() != num_envs or base_col.numel() != num_envs:
        raise ValueError("base_row and base_col must have shape [num_envs]")
    base_row = base_row.reshape(num_envs, 1, 1)
    base_col = base_col.reshape(num_envs, 1, 1)
    angles = torch.as_tensor(angles, device=device)
    if angles.ndim == 1:
        angles = angles.unsqueeze(0).expand(num_envs, -1)
    if angles.ndim != 2 or angles.shape[0] != num_envs:
        raise ValueError("angles must have shape [num_rays] or [num_envs, num_rays]")
    ...
    r_3d = r_vals.view(1, 1, num_steps)
    angles_3d = angles.unsqueeze(-1)
    row_3d = torch.round(base_row + r_3d * torch.cos(angles_3d)).long()
    col_3d = torch.round(base_col + r_3d * torch.sin(angles_3d)).long()
    ...
    env_idx = env_idx.expand(-1, row_3d.shape[1], row_3d.shape[2])
    grid_vals_3d = torch.zeros_like(row_3d, dtype=grid_batch.dtype)
    grid_vals_3d[valid_mask_3d] = grid_batch[
        env_idx[valid_mask_3d],
        row_clamped[valid_mask_3d],
        col_clamped[valid_mask_3d],
    ]
