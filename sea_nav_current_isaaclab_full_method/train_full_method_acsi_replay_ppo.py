...
parser.add_argument("--tb-flush-interval", type=int, default=50)
...
parser.add_argument(
    "--experiment-mode",
    choices=("fidelity_reproduction", "performance_optimization", "ablation"),
    default="fidelity_reproduction",
)
parser.add_argument("--enable-perf-profiling", action="store_true")
parser.add_argument("--perf-profile-interval", type=int, default=10)
parser.add_argument(
    "--perf-timing-mode",
    choices=("low_overhead", "sync_debug"),
...
if args.tb_flush_interval < 1:
    parser.error("--tb-flush-interval must be >= 1")
if args.perf_profile_interval < 1:
    parser.error("--perf-profile-interval must be >= 1")
...
class RootGridGoalRays:
    root_xy_local: "torch.Tensor"
    robot_cell: "torch.Tensor"
    yaw: "torch.Tensor"
    rays_m: "torch.Tensor"
    ...
    def __iter__(self):
        yield self.root_xy_local
        yield self.robot_cell
        yield self.yaw
        yield self.rays_m
        yield self.goal_local
        yield self.distance
...
def baseline_result_contract(args, adapter_env, sea_root, torch, result_class, runner=None):
    git_status = _git_output(["status", "--short"], sea_root)
    cuda_device_name = torch.cuda.get_device_name(0) if torch.cuda.is_available() else None
    if adapter_env is not None and hasattr(adapter_env, "materialize_debug_state"):
        adapter_env.materialize_debug_state()
    perf_profile = adapter_env.get_perf_profile(runner=runner) if adapter_env is not None else {}
    return {
        "git_commit": _git_output(["rev-parse", "HEAD"], sea_root),
        "git_dirty": bool(git_status),
        "torch_version": torch.__version__,
        "isaac_lab_version": _package_version("isaaclab", "isaaclab_tasks"),
        "isaac_sim_version": _package_version("isaacsim", "isaacsim-app", "isaacsim-extscache-kit"),
        "cuda_device_name": cuda_device_name,
        "num_envs": int(args.num_envs),
        "rollout_steps": int(args.rollout_steps),
        "ppo_num_learning_epochs": int(args.ppo_num_learning_epochs),
        "ppo_num_mini_batches": int(args.ppo_num_mini_batches),
        "save_interval": int(args.save_interval),
        "tb_flush_interval": int(args.tb_flush_interval),
        "experiment_mode": args.experiment_mode,
        "result_class": result_class,
        "perf_timing_mode": args.perf_timing_mode,
        "perf_profiling_enabled": bool(args.enable_perf_profiling),
        "perf_profile_interval": int(args.perf_profile_interval),
        "perf_profile": perf_profile,
        "sync_debug_formal_report_allowed": False,
        "raycast_impl_version": "batched_variable_origins_step_reuse_v1",
        "replay_impl_version": "collision_onset_legacy_python_ring_v1",
        ...
        "cache_impl_version": "fov_contact_indices_cache_v1",
        "debug_materialization_impl_version": "debug_materialization_guard_v1",
        "debug_materialization_guard_enabled": True,
        "debug_materialization_enabled": bool(getattr(adapter_env, "debug_materialization_enabled", False)),
        ...
        "raycast_vectorized_enabled": bool(getattr(adapter_env, "raycast_vectorized_enabled", False)),
        "raycast_fallback_count": int(getattr(adapter_env, "raycast_fallback_count", 0)),
        "raycast_call_count": int(getattr(adapter_env, "raycast_call_count", 0)),
        "raycast_calls_per_env_step_mean": float(getattr(adapter_env, "raycast_calls_per_env_step_mean", 0.0)),
        ...
        "replay_fallback_reset_count": int(getattr(adapter_env, "replay_fallback_reset_count", 0)),
...
class TrainerCommandFilter:
    ...
    def step(self, command, *, debug: bool = False):
        filtered, debug_info = self.queue_filter.step(command, debug=debug)
        self._filtered = filtered.detach().clone()
        if not debug:
            return self._filtered.clone(), {}
        debug_info["mode"] = self.mode
        debug_info["bypassed_queue_delay"] = True
        return self._filtered.clone(), {
            **debug_info,
            "mode": self.mode,
...
class SeaNavOriginalSemanticsIsaacLabEnv:
    def __init__(
        ...
        if args.iterations > 0 and not self.raycast_vectorized_enabled:
            raise RuntimeError(
                "training requires grid2ray.batch_ray_cast_torch_variable_origins; "
                "fallback per-env raycast is smoke/debug only"
            )
        self.resolution = 0.1
        ...
        self._ray_indices_all = torch.arange(self.ray_angles.numel(), dtype=torch.long, device=self.device)
        ray_abs = torch.abs(self.ray_angles)
        self._ray_indices_fov120 = self._ray_indices_all[ray_abs <= (120.0 / 2.0) * math.pi / 180.0]
        self._ray_mask_fov150 = ray_abs <= (150.0 / 2.0) * math.pi / 180.0
        self._cached_contact_indices = None
        self.fov_index_cache_enabled = True
        ...
        self.replay_invalid_sample_count = 0
        self.replay_fallback_reset_count = 0
        ...
        self.raycast_call_count = 0
        self.raycast_fallback_count = 0
        self.raycast_non_reset_path_call_count = 0
        self.raycast_non_reset_path_step_count = 0
        self.raycast_calls_per_env_step_mean = 0.0
        self.enable_perf_profiling = bool(args.enable_perf_profiling)
        self.perf_timing_mode = args.perf_timing_mode
        self.perf_profile_interval = int(args.perf_profile_interval)
        self.perf_counters = {
            "raycast_time_s": 0.0,
            "update_observation_time_s": 0.0,
            "capture_replay_record_time_s": 0.0,
            "replay_push_time_s": 0.0,
            "reset_total_time_s": 0.0,
            "replay_restore_time_s": 0.0,
            "restore_replay_sample_time_s": 0.0,
        }
        ...
        self.rays_hist = torch.ones(self.num_envs, self.hist_len, 41, device=self.device) * 5.0
        self.goal_hist = torch.zeros(self.num_envs, self.hist_len, 2, device=self.device)
        ...
        self.sea_obs_hist = torch.zeros(self.num_envs, self.hist_len, 55, device=self.device)
        self.slr_obs_hist = torch.zeros(self.num_envs, self.hist_len, 45, device=self.device)
        self.pos_hist = torch.zeros(self.num_envs, self.hist_len, 2, device=self.device)
        ...
        self.debug_materialization_enabled = bool(
            args.force_replay_smoke
            or args.force_source_parity_smoke
            or args.force_reward_done_parity_smoke
            or args.force_go2_walk_smoke
            or (args.enable_perf_profiling and args.perf_timing_mode == "sync_debug")
        )
    ...
    def _ordered_history(self, hist, write_index, env_ids=None):
        ...
        hist_env = hist[env_ids]
        hist_len = int(hist.shape[1])
        offsets = torch.arange(hist_len, dtype=torch.long, device=self.device).unsqueeze(0)
        slots = (write_index[env_ids].reshape(-1, 1) + offsets).remainder(hist_len)
        index = slots.reshape(slots.shape[0], hist_len, *([1] * (hist_env.ndim - 2))).expand_as(hist_env)
        return torch.gather(hist_env, dim=1, index=index)

    def _bootstrap_history(self, hist, write_index, env_ids, value):
        env_ids = self._env_ids_tensor(env_ids)
        if env_ids.numel() == 0:
            return
        value = value.to(device=self.device, dtype=hist.dtype)
        hist_len = int(hist.shape[1])
        hist[env_ids] = value.unsqueeze(1).expand(-1, hist_len, *value.shape[1:])
        write_index[env_ids] = 0
    ...
    def _write_history_ring(self, hist, write_index, env_ids, new_value):
        ...
        if env_ids.numel() == 0:
            return
        hist_len = int(hist.shape[1])
        new_value = new_value.to(device=self.device, dtype=hist.dtype)
        slots = write_index[env_ids]
        hist[env_ids, slots] = new_value
        write_index[env_ids] = (slots + 1).remainder(hist_len)

    def _write_history_ring_pair(self, first_hist, second_hist, write_index, env_ids, first_value, second_value):
        env_ids = self._env_ids_tensor(env_ids)
        if env_ids.numel() == 0:
            return
        hist_len = int(first_hist.shape[1])
        first_value = first_value.to(device=self.device, dtype=first_hist.dtype)
        second_value = second_value.to(device=self.device, dtype=second_hist.dtype)
        slots = write_index[env_ids]
        first_hist[env_ids, slots] = first_value
        second_hist[env_ids, slots] = second_value
        write_index[env_ids] = (slots + 1).remainder(hist_len)
    ...
    def _restore_chronological_history(self, hist, write_index, env_ids, value):
        ...
        if env_ids.numel() == 0:
            return
        restored = self._sample_tensor(value, dtype=hist.dtype).reshape(
            len(env_ids),
            int(hist.shape[1]),
            *hist.shape[2:],
        )
        hist[env_ids] = restored
        write_index[env_ids] = 0

    def _materialize_reset_env_ids(self, replay_mask, normal_mask):
        replay_mask = replay_mask.to(device=self.device, dtype=self.torch.bool).flatten()
        normal_mask = normal_mask.to(device=self.device, dtype=self.torch.bool).flatten()
        if replay_mask.numel() != self.num_envs or normal_mask.numel() != self.num_envs:
            raise ValueError("reset masks must have shape [num_envs]")
        ...
        return replay_env_ids, normal_env_ids
    ...
    @property
    def contact_indices_cached(self):
        return getattr(self, "_cached_contact_indices", None) is not None

    def _perf_sync_if_requested(self):
        if (
            self.enable_perf_profiling
            and self.perf_timing_mode == "sync_debug"
            and getattr(self.device, "type", "") == "cuda"
            and self.torch.cuda.is_available()
        ):
            self.torch.cuda.synchronize(self.device)

    def _perf_begin(self):
        if not self.enable_perf_profiling:
            return None
        self._perf_sync_if_requested()
        return time.perf_counter()

    def _perf_end(self, name, started_at):
        if started_at is None:
            return
        self._perf_sync_if_requested()
        self.perf_counters[name] += time.perf_counter() - started_at

    def get_perf_profile(self, runner=None):
        counters = dict(self.perf_counters)
        counters.update(
            {
                ...
                "replay_invalid_sample_count": int(getattr(self, "replay_invalid_sample_count", 0)),
                "replay_fallback_reset_count": int(getattr(self, "replay_fallback_reset_count", 0)),
                "replay_fallback_count": int(getattr(self, "replay_fallback_count", 0)),
                "done_count": int(self.semantic_done_count),
                "collision_onset_count": int(self.replay_collision_record_count),
                "raycast_call_count": int(self.raycast_call_count),
                "raycast_fallback_count": int(self.raycast_fallback_count),
                "raycast_calls_per_env_step_mean": float(self.raycast_calls_per_env_step_mean),
            }
        )
        runner_summary = getattr(runner, "perf_summary", None) if runner is not None else None
        return {
            "enabled": self.enable_perf_profiling,
            "timing_mode": self.perf_timing_mode,
            "profile_interval": self.perf_profile_interval,
            "low_overhead_default": self.perf_timing_mode == "low_overhead",
            "uses_cuda_synchronize": self.enable_perf_profiling and self.perf_timing_mode == "sync_debug",
            "counters": counters,
            "runner": runner_summary or {},
        }
    ...
    def _build_reset_debug(self, root_pose, root_velocity, env_ids=None, root_grid_goal_rays=None):
        env_ids = self._env_ids_tensor(env_ids)
        if root_grid_goal_rays is None:
            root_grid_goal_rays = self._root_grid_goal_rays()
        robot_cell = root_grid_goal_rays.robot_cell
        yaw = root_grid_goal_rays.yaw
        distance = root_grid_goal_rays.distance
        return {
    ...
    def _snapshot_task_state(self):
        return {
            ...
            "rays_hist": self._ordered_history(self.rays_hist, self.ray_goal_hist_write_index).detach().clone(),
            "goal_hist": self._ordered_history(self.goal_hist, self.ray_goal_hist_write_index).detach().clone(),
            "delay_rays": self.delay_rays.detach().clone(),
            "delay_goal": self.delay_goal.detach().clone(),
            "pos_hist": self._ordered_history(self.pos_hist, self.pos_hist_write_index).detach().clone(),
            "slr_command": self.slr_command.detach().clone(),
            "last_loco_action": self.last_loco_action.detach().clone(),
            "command_filter_filtered": self.command_filter.filtered.detach().clone(),
            "command_queue_filtered": self.command_filter.queue_filter.filtered.detach().clone(),
            ...
            "last_collision_active": self.last_collision_active.detach().clone(),
            "last_replay_eligible": self.last_replay_eligible.detach().clone(),
        }

    def _restore_replay_sample(self, replay_sample, env_ids=None):
        perf_start = self._perf_begin()
        env_ids = self._env_ids_tensor(env_ids)
        try:
            robot = self._robot()
            ...
            self._perf_end("restore_replay_sample_time_s", perf_start)

    def _replay_batch_safe_mask(self, replay_batch):
        torch = self.torch
        env_ids = self._env_ids_tensor(replay_batch.env_ids)
        batch_len = int(env_ids.numel())
        valid_mask = replay_batch.valid_mask.to(device=self.device, dtype=torch.bool).flatten()
        if valid_mask.numel() != batch_len:
            return torch.zeros(batch_len, dtype=torch.bool, device=self.device)
        ...
        required_task_shapes = {
            ...
            "slr_command": (3,),
            "last_loco_action": (12,),
            "command_filter_filtered": (3,),
            "command_queue_filtered": (3,),
            "goal_hold_timer": (),
            "stay_timer": (),
            "episode_length_buf": (),
            "reset_buf": (),
        ...
        try:
            for name, expected_shape in required_shapes.items():
                tensor = getattr(replay_batch, name)
                if not isinstance(tensor, torch.Tensor) or int(tensor.shape[0]) != batch_len:
                    return torch.zeros(batch_len, dtype=torch.bool, device=self.device)
                if tuple(tensor.shape[1:]) != expected_shape:
                    return torch.zeros(batch_len, dtype=torch.bool, device=self.device)
                if tensor.is_floating_point():
                    safe_mask &= torch.isfinite(tensor.reshape(batch_len, -1)).all(dim=1).to(self.device)
            task_state = replay_batch.task_state
            for name, expected_shape in required_task_shapes.items():
                tensor = task_state.get(name)
                if not isinstance(tensor, torch.Tensor) or int(tensor.shape[0]) != batch_len:
                    return torch.zeros(batch_len, dtype=torch.bool, device=self.device)
                if tuple(tensor.shape[1:]) != expected_shape:
                    return torch.zeros(batch_len, dtype=torch.bool, device=self.device)
    ...
    def _restore_replay_batch_adapter_state(self, replay_batch, env_ids, source_indices):
        torch = self.torch
        task_state = replay_batch.task_state
        self.map_origin_cell[env_ids] = task_state["map_origin_cell"][source_indices].to(
            device=self.device, dtype=torch.float32
        )
        self.start_cell[env_ids] = task_state["start_cell"][source_indices].to(device=self.device, dtype=torch.float32)
        self.goal_cell[env_ids] = task_state["goal_cell"][source_indices].to(device=self.device, dtype=torch.float32)
        self._restore_chronological_history(
            self.sea_obs_hist, self.sea_hist_write_index, env_ids, replay_batch.sea_obs_hist[source_indices]
        )
        self._restore_chronological_history(
            self.slr_obs_hist, self.slr_hist_write_index, env_ids, replay_batch.slr_obs_hist[source_indices]
        )
        self._restore_chronological_history(
            self.rays_hist, self.ray_goal_hist_write_index, env_ids, task_state["rays_hist"][source_indices]
        )
        self._restore_chronological_history(
            self.goal_hist, self.ray_goal_hist_write_index, env_ids, task_state["goal_hist"][source_indices]
        )
        self.delay_rays[env_ids] = task_state["delay_rays"][source_indices].to(device=self.device, dtype=torch.float32)
        self.delay_goal[env_ids] = task_state["delay_goal"][source_indices].to(device=self.device, dtype=torch.float32)
        self._restore_chronological_history(
            self.pos_hist, self.pos_hist_write_index, env_ids, task_state["pos_hist"][source_indices]
        )
        self.slr_command[env_ids] = task_state["slr_command"][source_indices].to(device=self.device, dtype=torch.float32)
        self.last_loco_action[env_ids] = task_state["last_loco_action"][source_indices].to(
            device=self.device, dtype=torch.float32
        )
        self.goal_hold_timer[env_ids] = task_state["goal_hold_timer"][source_indices].to(
            device=self.device, dtype=torch.long
        )
        self.stay_timer[env_ids] = task_state["stay_timer"][source_indices].to(device=self.device, dtype=torch.long)
        self.episode_length_buf[env_ids] = task_state["episode_length_buf"][source_indices].to(
            device=self.device, dtype=torch.long
        )
        self.reset_buf[env_ids] = task_state["reset_buf"][source_indices].to(device=self.device, dtype=torch.bool)
        self.collision_occurred[env_ids] = task_state["collision_occurred"][source_indices].to(
            device=self.device, dtype=torch.bool
        )
        self.last_collision_active[env_ids] = task_state["last_collision_active"][source_indices].to(
            device=self.device, dtype=torch.bool
        )
        self.last_replay_eligible[env_ids] = task_state["last_replay_eligible"][source_indices].to(
            device=self.device, dtype=torch.bool
        )
        self.command_filter.reset(env_ids=env_ids)
        self.command_filter._filtered[env_ids] = task_state["command_filter_filtered"][source_indices].to(
            device=self.device, dtype=torch.float32
        )
        self.command_filter.queue_filter.filtered[env_ids] = task_state["command_queue_filtered"][source_indices].to(
            device=self.device, dtype=torch.float32
        )
    ...
    def _reset_replay_batch_conservative(self, replay_batch, safe_mask):
        torch = self.torch
        env_ids = self._env_ids_tensor(replay_batch.env_ids)
        # PERF_SYNC_BOUNDARY: replay fast reset safe-row materialization
        source_indices = safe_mask.nonzero(as_tuple=False).flatten()
        if source_indices.numel() == 0:
            return torch.empty(0, dtype=torch.long, device=self.device)
        fast_env_ids = env_ids[source_indices]
        robot = self._robot()
        root_pose = replay_batch.root_pose[source_indices].to(device=self.device, dtype=torch.float32)
        root_velocity = replay_batch.root_velocity[source_indices].to(device=self.device, dtype=torch.float32)
        dof_pos = replay_batch.dof_pos[source_indices].to(device=self.device, dtype=torch.float32)
        dof_vel = replay_batch.dof_vel[source_indices].to(device=self.device, dtype=torch.float32)
        ...
        self._restore_replay_batch_adapter_state(replay_batch, fast_env_ids, source_indices)
        self.carrier.unwrapped.scene.write_data_to_sim()
        self.carrier.unwrapped.sim.forward()
        self.carrier.unwrapped.scene.update(dt=self.carrier.unwrapped.physics_dt)
        ...
        root_grid_goal_rays = self._root_grid_goal_rays()
        restored = int(fast_env_ids.numel())
        self.obs_buf[fast_env_ids] = self._ordered_history(
            self.sea_obs_hist, self.sea_hist_write_index, fast_env_ids
        ).reshape(restored, -1)
        self.last_source_reset_debug = (
            self._build_reset_debug(
                root_pose,
                root_velocity,
                env_ids=fast_env_ids,
                root_grid_goal_rays=root_grid_goal_rays,
            )
            if self.debug_materialization_enabled
            else {}
        )
        self.replay_fast_reset_count += restored
        self.replay_reset_count += restored
    ...
    def reset_replay_batch(self, replay_batch):
        ...
        if batch_len == 0:
            self.last_replay_batch_debug = {
                "replay_fast_reset_enabled": self.replay_fast_reset_enabled,
                "replay_fast_reset_mode": self.replay_fast_reset_mode,
                "requested_env_count": 0,
                "valid_sample_count": 0,
                "fast_reset_count": 0,
                "legacy_reset_count": 0,
                "normal_reset_count": 0,
                "fallback_reset_count": 0,
            }
            return self.last_replay_batch_debug

        valid_mask = replay_batch.valid_mask.to(device=self.device, dtype=torch.bool).flatten()
        if valid_mask.numel() != batch_len:
            valid_mask = torch.zeros(batch_len, dtype=torch.bool, device=self.device)
        normal_mask = ~valid_mask
        fast_restored_mask = torch.zeros(batch_len, dtype=torch.bool, device=self.device)
        fast_error = None

        if normal_mask.any():  # PERF_SYNC_BOUNDARY: replay invalid normal-reset fallback decision
            self.replay_invalid_sample_count += int(normal_mask.long().sum().detach().cpu())
            self.replay_fallback_count += int(normal_mask.long().sum().detach().cpu())
            self.reset(env_ids=env_ids[normal_mask])

        if self.replay_fast_reset_enabled:
            safe_mask = self._replay_batch_safe_mask(replay_batch)
            try:
                restored_indices = self._reset_replay_batch_conservative(replay_batch, safe_mask)
                if restored_indices.numel() != 0:
                    fast_restored_mask[restored_indices] = True
                    self.replay_sample_count += int(restored_indices.numel())
            except (AttributeError, KeyError, RuntimeError, TypeError, ValueError) as exc:
                fast_error = f"{type(exc).__name__}: {exc}"
                fast_restored_mask.zero_()

        legacy_mask = valid_mask & ~fast_restored_mask
        legacy_reset_count = 0
        fallback_reset_count = 0
        if legacy_mask.any():  # PERF_SYNC_BOUNDARY: legacy replay reset compatibility materialization
            legacy_mask_cpu = legacy_mask.detach().cpu().tolist()
            for sample_index, sample_valid in enumerate(legacy_mask_cpu):
                if not bool(sample_valid):
                    continue
                replay_sample = replay_batch.to_legacy_sample(sample_index)
                self.replay_sample_count += 1
                legacy_reset_count += 1
                if self.replay_fast_reset_enabled:
                    self.replay_fallback_reset_count += 1
                    self.replay_fallback_count += 1
                    fallback_reset_count += 1
                self.reset(env_ids=env_ids[sample_index : sample_index + 1], replay_sample=replay_sample)

        # PERF_SYNC_BOUNDARY: replay batch debug/result materialization
        valid_count = int(valid_mask.long().sum().detach().cpu())
        normal_count = int(normal_mask.long().sum().detach().cpu())
        fast_count = int(fast_restored_mask.long().sum().detach().cpu())
        self.last_replay_batch_debug = {
            "replay_fast_reset_enabled": self.replay_fast_reset_enabled,
            "replay_fast_reset_mode": self.replay_fast_reset_mode,
            "requested_env_count": batch_len,
            "valid_sample_count": valid_count,
            "fast_reset_count": fast_count,
            "legacy_reset_count": int(legacy_reset_count),
            "normal_reset_count": normal_count,
            "fallback_reset_count": int(fallback_reset_count),
            "fast_reset_error": fast_error,
        }
        return self.last_replay_batch_debug
    ...
    def _capture_replay_record(self, collision=False):
        ...
        replay_push_start = self._perf_begin()
        try:
            self.replay_buffer.push(
                ...
                sea_obs_hist=self._ordered_history(self.sea_obs_hist, self.sea_hist_write_index).detach().clone(),
                slr_obs_hist=self._ordered_history(self.slr_obs_hist, self.slr_hist_write_index).detach().clone(),
                task_state=self._snapshot_task_state(),
                collision=collision,
                owned=True,
            )
        finally:
            self._perf_end("replay_push_time_s", replay_push_start)
            self._perf_end("capture_replay_record_time_s", perf_start)
    ...
    def run_forced_replay_reset_smoke(self):
        ...
        replay_env_ids = self.torch.arange(self.num_envs, dtype=self.torch.long, device=self.device)
        replay_undo_steps = self.torch.full(
            (self.num_envs,),
            int(undo_steps),
            dtype=self.torch.long,
            device=self.device,
        )
        replay_batch = self.replay_buffer.sample_pre_collision(env_ids=replay_env_ids, undo_steps=replay_undo_steps)
        per_env = []
        # PERF_SYNC_BOUNDARY: forced replay smoke reset fallback materialization
        valid_mask_cpu = replay_batch.valid_mask.detach().cpu().tolist()
        step_index_cpu = replay_batch.step_index.detach().cpu().tolist()
        source_collision_cpu = replay_batch.source_collision_step.detach().cpu().tolist()
        undo_steps_cpu = replay_batch.undo_steps.detach().cpu().tolist()
        stored_steps_cpu = [self.replay_buffer.stored_steps_for_env(env_id) for env_id in range(self.num_envs)]
        self.reset_replay_batch(replay_batch)
        for sample_index, env_id in enumerate(range(self.num_envs)):
            per_env.append(
                {
                    "env_id": env_id,
                    "sample_found": bool(valid_mask_cpu[sample_index]),
                    "sample_step_index": int(step_index_cpu[sample_index]) if bool(valid_mask_cpu[sample_index]) else None,
                    "source_collision_step": int(source_collision_cpu[sample_index])
                    if bool(valid_mask_cpu[sample_index])
                    else None,
                    "undo_steps": int(undo_steps_cpu[sample_index]),
                    "stored_steps": stored_steps_cpu[env_id],
                }
            )
        ...
        result = {
            "ok": bool(
                self.num_envs == 4
                and all_samples_found
                and self.replay_invalid_sample_count == 0
                and self.replay_fallback_reset_count == 0
                and self.torch.isfinite(self.obs_buf).all().detach().cpu()
            ),
            "forced": True,
            "expected_num_envs": 4,
            ...
            "replay_sample_count": int(self.replay_sample_count),
            "replay_invalid_sample_count": int(self.replay_invalid_sample_count),
            "replay_fallback_reset_count": int(self.replay_fallback_reset_count),
            "replay_fallback_count": int(self.replay_fallback_count),
            "replay_fast_reset_enabled": bool(self.replay_fast_reset_enabled),
            "replay_fast_reset_mode": self.replay_fast_reset_mode,
            "replay_fast_reset_count": int(self.replay_fast_reset_count),
            "last_replay_batch_debug": self.last_replay_batch_debug,
            "done_reset_impl_version": "mask_first_boundary_sync_v1",
            "replay_sync_boundary": "batch_reset_only",
            "history_ring_impl_version": self.history_ring_impl_version,
            "history_ring_enabled": bool(self.history_ring_enabled),
            "history_ring_chronological_outputs": True,
    ...
    def _root_grid_goal_rays(self):
        ...
        try:
            ...
            if self.raycast_vectorized_enabled:
                ...
                if args.iterations > 0:
                    raise RuntimeError(
                        "training fallback to per-env raycast is disabled; "
                        "install batch_ray_cast_torch_variable_origins"
                    )
                rays_cells = []
            ...
            return RootGridGoalRays(
                root_xy_local=root_xy_local,
            ...
            self._perf_end("raycast_time_s", perf_start)

    def _clearance(self, rays_m, fov_deg=None):
        torch = self.torch
        if fov_deg is None:
            indices = self._ray_indices_all
        elif float(fov_deg) == 120.0:
            indices = self._ray_indices_fov120
        else:
            angle_threshold = (fov_deg / 2.0) * math.pi / 180.0
            indices = self._ray_indices_all[torch.abs(self.ray_angles) <= angle_threshold]
    ...
    def _guidance_nav_alignment(self, rays_m, fov_deg=150.0):
        ...
        if float(fov_deg) == 150.0:
            mask = self._ray_mask_fov150
        else:
            angle_threshold = (fov_deg / 2.0) * math.pi / 180.0
            mask = torch.abs(self.ray_angles) <= angle_threshold
        scores = torch.where(mask, smoothed, smoothed.new_full((), -1.0)) - torch.abs(self.ray_angles) * 0.001
    ...
    def _contact_indices(self):
        if self._cached_contact_indices is not None:
            return self._cached_contact_indices
        ...
        self._cached_contact_indices = (names, base, head, thigh, calf, foot)
        return self._cached_contact_indices
    ...
    def _update_observation(self, env_ids=None, root_grid_goal_rays=None):
        ...
        try:
            ...
            env_ids_tensor = self._env_ids_tensor(env_ids)
            rays_m = root_grid_goal_rays.rays_m
            goal_local = root_grid_goal_rays.goal_local
            base_lin_vel = self.policy_obs[:, 0:3]
            base_ang_vel = self.policy_obs[:, 3:6]
            projected_gravity = self.policy_obs[:, 6:9]
            initial_mask = self.episode_length_buf[env_ids_tensor] <= 1
            initial_env_ids = env_ids_tensor[initial_mask]
            rolling_env_ids = env_ids_tensor[~initial_mask]
            self._bootstrap_history(
                self.rays_hist, self.ray_goal_hist_write_index, initial_env_ids, rays_m[initial_env_ids]
            )
            self._bootstrap_history(
                self.goal_hist, self.ray_goal_hist_write_index, initial_env_ids, goal_local[initial_env_ids]
            )
            self._write_history_ring_pair(
                self.rays_hist,
                self.goal_hist,
                self.ray_goal_hist_write_index,
                rolling_env_ids,
                rays_m[rolling_env_ids],
                goal_local[rolling_env_ids],
            )
            if self.source_perception_delay_enabled:
                ...
                if len(env_ids_to_update) != 0:
                    ...
                    ordered_rays = self._ordered_history(
                        self.rays_hist, self.ray_goal_hist_write_index, env_ids_to_update
                    )
                    ordered_goal = self._ordered_history(
                        self.goal_hist, self.ray_goal_hist_write_index, env_ids_to_update
                    )
                    delay_rows = torch.arange(len(env_ids_to_update), dtype=torch.long, device=self.device)
                    self.delay_rays[env_ids_to_update] = ordered_rays[delay_rows, resample_idx, :]
                    self.delay_goal[env_ids_to_update] = ordered_goal[delay_rows, resample_idx, :]
                    if self.debug_materialization_enabled:
            ...
            self._bootstrap_history(
                self.sea_obs_hist, self.sea_hist_write_index, initial_env_ids, one_step[initial_env_ids]
            )
            self._write_history_ring(
                self.sea_obs_hist, self.sea_hist_write_index, rolling_env_ids, one_step[rolling_env_ids]
            )
            self.obs_buf[env_ids_tensor] = self._ordered_history(
                self.sea_obs_hist, self.sea_hist_write_index, env_ids_tensor
            ).reshape(len(env_ids_tensor), -1)
        finally:
            self._perf_end("update_observation_time_s", perf_start)
    ...
    def _update_position_history(self, root_xy, *, debug: bool = False):
        torch = self.torch
        all_env_ids = self._env_ids_tensor()
        if self.source_reward_done_parity_enabled:
            update_mask = self.episode_length_buf % self.source_pos_hist_interval_steps == 0
            update_env_ids = all_env_ids[update_mask]
            if update_env_ids.numel() != 0:
                initial_mask = self.episode_length_buf[update_env_ids] <= 1
                initial_env_ids = update_env_ids[initial_mask]
                rolling_env_ids = update_env_ids[~initial_mask]
                self._bootstrap_history(
                    self.pos_hist, self.pos_hist_write_index, initial_env_ids, root_xy[initial_env_ids]
                )
                self._write_history_ring(
                    self.pos_hist, self.pos_hist_write_index, rolling_env_ids, root_xy[rolling_env_ids]
                )
            if not debug:
                return {}
            # PERF_SYNC_BOUNDARY: debug-only position-history env_ids materialization
            updated_env_ids = torch.where(update_mask)[0]
            return {
                "source_reward_done_parity_enabled": True,
                "pos_hist_interval_steps": int(self.source_pos_hist_interval_steps),
                "updated_env_count": update_mask.long().sum().detach().clone(),
                "updated_env_ids": updated_env_ids.detach().clone(),
            }
        self._write_history_ring(
            self.pos_hist, self.pos_hist_write_index, all_env_ids, root_xy[all_env_ids]
        )
        if not debug:
            return {}
        return {
    ...
    def run_reward_done_parity_contract_smoke(self):
        ...
        try:
            ...
            self.pos_hist_write_index = original_pos_hist_write_index
    ...
    def reset(self, env_ids=None, replay_sample=None):
        reset_perf_start = self._perf_begin()
        torch = self.torch
        ...
        if self.source_parity_reset_enabled and not is_replay:
            start_cell, goal_cell, reset_yaw = self._sample_source_reset_cells_and_yaw_at_reset_boundary(
                env_ids_tensor
            )
            ...
            reset_yaw = None
        if is_replay:
            replay_restore_perf_start = self._perf_begin()
            self._restore_replay_sample(replay_sample, env_ids=env_ids_tensor)
            self._perf_end("replay_restore_time_s", replay_restore_perf_start)
            root_state = replay_sample["root_state"]
            if isinstance(root_state, dict):
        ...
        self.sea_hist_write_index[env_ids_tensor] = 0
        self.slr_hist_write_index[env_ids_tensor] = 0
        self.ray_goal_hist_write_index[env_ids_tensor] = 0
        self.pos_hist_write_index[env_ids_tensor] = 0
        ...
        if is_replay:
            ...
            self._restore_chronological_history(
                self.sea_obs_hist, self.sea_hist_write_index, env_ids_tensor, replay_sample["sea_obs_hist"]
            )
            self._restore_chronological_history(
                self.slr_obs_hist, self.slr_hist_write_index, env_ids_tensor, replay_sample["slr_obs_hist"]
            )
            self._restore_chronological_history(
                self.rays_hist,
                self.ray_goal_hist_write_index,
                env_ids_tensor,
                task_state.get("rays_hist", self.rays_hist[env_ids_tensor]),
            )
            self._restore_chronological_history(
                self.goal_hist,
                self.ray_goal_hist_write_index,
                env_ids_tensor,
                task_state.get("goal_hist", self.goal_hist[env_ids_tensor]),
            )
            self.delay_rays[env_ids_tensor] = self._sample_tensor(
                task_state.get("delay_rays", self.delay_rays[env_ids_tensor]), dtype=torch.float32
            ).reshape(len(env_ids_tensor), 41)
            self.delay_goal[env_ids_tensor] = self._sample_tensor(
                task_state.get("delay_goal", self.delay_goal[env_ids_tensor]), dtype=torch.float32
            ).reshape(len(env_ids_tensor), 2)
            self._restore_chronological_history(
                self.pos_hist,
                self.pos_hist_write_index,
                env_ids_tensor,
                task_state.get("pos_hist", self.pos_hist[env_ids_tensor]),
            )
            self.slr_command[env_ids_tensor] = self._sample_tensor(
                task_state.get("slr_command", replay_sample["command"]), dtype=torch.float32
            ...
            self.reset_buf[env_ids_tensor] = self._sample_tensor(
                task_state.get("reset_buf", self.reset_buf[env_ids_tensor]),
                dtype=torch.bool,
            ).reshape(len(env_ids_tensor))
            ...
            self.last_collision_active[env_ids_tensor] = self._sample_tensor(
                task_state.get("last_collision_active", self.last_collision_active[env_ids_tensor]),
                dtype=torch.bool,
            ).reshape(len(env_ids_tensor))
            self.last_replay_eligible[env_ids_tensor] = self._sample_tensor(
                task_state.get("last_replay_eligible", self.last_replay_eligible[env_ids_tensor]),
                dtype=torch.bool,
            ).reshape(len(env_ids_tensor))
            self.command_filter._filtered[env_ids_tensor] = self._sample_tensor(
                task_state.get("command_filter_filtered", self.slr_command[env_ids_tensor]),
                dtype=torch.float32,
            ).reshape(len(env_ids_tensor), 3)
            self.command_filter.queue_filter.filtered[env_ids_tensor] = self._sample_tensor(
                task_state.get("command_queue_filtered", self.slr_command[env_ids_tensor]),
                dtype=torch.float32,
            ).reshape(len(env_ids_tensor), 3)
            root_grid_goal_rays = self._root_grid_goal_rays()
            self.obs_buf[env_ids_tensor] = self._ordered_history(
            ...
            root_grid_goal_rays = self._root_grid_goal_rays()
            root_xy = root_grid_goal_rays.root_xy_local
            self._bootstrap_history(self.pos_hist, self.pos_hist_write_index, env_ids_tensor, root_xy[env_ids_tensor])
            self._update_observation(env_ids=env_ids_tensor, root_grid_goal_rays=root_grid_goal_rays)
        self.last_source_reset_debug = (
            self._build_reset_debug(
                root_pose,
                root_velocity,
        ...
        self.reset_count += 1
        self._perf_end("reset_total_time_s", reset_perf_start)
        return self.obs_buf, self.privileged_obs_buf

    def _compute_reward_done(self, root_grid_goal_rays=None, return_collision_onset=False):
        torch = self.torch
        if root_grid_goal_rays is None:
            raise ValueError("root_grid_goal_rays must be computed once by the caller")
        root_xy = root_grid_goal_rays.root_xy_local
        ...
        pos_hist_debug = self._update_position_history(root_xy, debug=self.debug_materialization_enabled)
        ordered_pos_hist = self._ordered_history(self.pos_hist, self.pos_hist_write_index)
        distances_hist = torch.norm(ordered_pos_hist - root_xy[:, None, :], dim=-1)
        ...
        if self.source_reward_done_parity_enabled:
            d_low = torch.norm(root_xy - ordered_pos_hist[:, 0, :], dim=-1) < 0.2
        else:
        ...
        if return_collision_onset:
            return (*result, collision_onset.detach().clone())
        return result

    def step(self, actions):
        torch = self.torch
        nav_action_orig = actions.detach().clip(-3.0, 3.0)
        self.slr_command, self.last_delay_debug = self.command_filter.step(
            nav_action_orig,
        ...
        raycast_calls_before_step = self.raycast_call_count
        root_grid_goal_rays = self._root_grid_goal_rays()
        reward, semantic_done, infos, collision_onset = self._compute_reward_done(
            root_grid_goal_rays=root_grid_goal_rays,
        ...
        self.raycast_non_reset_path_call_count += self.raycast_call_count - raycast_calls_before_step
        self.raycast_non_reset_path_step_count += 1
        self.raycast_calls_per_env_step_mean = (
            self.raycast_non_reset_path_call_count / max(1, self.raycast_non_reset_path_step_count)
        )
        next_obs = self.obs_buf.clone()
        done_mask = done.bool()
        eligible_mask = done_mask & replay_eligible
        replay_draw = torch.rand(self.num_envs, device=self.device)
        if self.collision_replay_config.enabled_during_training:
            ...
            use_replay_mask = torch.zeros_like(done_mask)
    ...
    def _step_carrier_with_low_level_command(self, command):
        ...
        if self.low_level_controller == "sea_nav_jit":
            ...
            all_env_ids = self._env_ids_tensor()
            initial_mask = self.episode_length_buf <= 1
            initial_env_ids = all_env_ids[initial_mask]
            rolling_env_ids = all_env_ids[~initial_mask]
            self._bootstrap_history(self.slr_obs_hist, self.slr_hist_write_index, initial_env_ids, slr_obs[initial_env_ids])
            self._write_history_ring(self.slr_obs_hist, self.slr_hist_write_index, rolling_env_ids, slr_obs[rolling_env_ids])
            hist_flat = self._ordered_history(self.slr_obs_hist, self.slr_hist_write_index).reshape(self.num_envs, -1)
    ...
    def run_go2_walk_smoke(self, steps, command_xyz, min_forward_m, min_speed_mps, min_height_m, max_height_m):
        ...
        for _ in range(int(steps)):
            ...
            if bool((terminated_any | truncated_any).any().detach().cpu()):  # smoke-only early stop
                break
...
def main():
    ...
    if args.force_go2_walk_smoke:
        ...
        if args.iterations <= 0:
            result = {
                ...
                **baseline_result_contract(args, adapter_env, sea_root, torch, result_class="go2_walk_smoke"),
    ...
    if args.force_reward_done_parity_smoke:
        ...
        if args.iterations <= 0:
            result = {
                ...
                "cuda_available": torch.cuda.is_available(),
                "device": str(adapter_env.device),
                **baseline_result_contract(args, adapter_env, sea_root, torch, result_class="reward_done_parity_smoke"),
                "asset_resolution": asset_resolution,
                "iterations": args.iterations,
                "rollout_steps": args.rollout_steps,
                "num_envs": args.num_envs,
    ...
    if args.force_source_parity_smoke:
        ...
        if args.iterations <= 0:
            result = {
                ...
                "cuda_available": torch.cuda.is_available(),
                "device": str(adapter_env.device),
                **baseline_result_contract(args, adapter_env, sea_root, torch, result_class="source_parity_smoke"),
                "asset_resolution": asset_resolution,
                "iterations": args.iterations,
                "rollout_steps": args.rollout_steps,
                "num_envs": args.num_envs,
    ...
    if args.force_replay_smoke:
        ...
        if args.iterations <= 0:
            result = {
                ...
                "cuda_available": torch.cuda.is_available(),
                "device": str(adapter_env.device),
                **baseline_result_contract(args, adapter_env, sea_root, torch, result_class="forced_replay_smoke"),
                "asset_resolution": asset_resolution,
                "iterations": args.iterations,
                "rollout_steps": args.rollout_steps,
                "num_envs": args.num_envs,
                ...
                **low_level_controller_contract(args),
                ...
                "replay_invalid_sample_count": adapter_env.replay_invalid_sample_count,
                "replay_fallback_reset_count": adapter_env.replay_fallback_reset_count,
    ...
    train_cfg = {
        "runner": {
            ...
            "tb_flush_interval": args.tb_flush_interval,
        },
        "algorithm": {
    ...
    runner = OnPolicyRunner(
        ...
        args=SimpleNamespace(
            wandb=False,
            enable_perf_profiling=args.enable_perf_profiling,
            perf_profile_interval=args.perf_profile_interval,
            perf_timing_mode=args.perf_timing_mode,
        ),
        device=adapter_env.device,
    )
    ...
    result = {
        ...
        "cuda_available": torch.cuda.is_available(),
        "device": str(adapter_env.device),
        **baseline_result_contract(args, adapter_env, sea_root, torch, result_class="training_contract_surface", runner=runner),
        "asset_resolution": asset_resolution,
        "iterations": args.iterations,
        ...
        **low_level_controller_contract(args),
        ...
        "history_ring_impl_version": adapter_env.history_ring_impl_version,
        "history_ring_enabled": bool(adapter_env.history_ring_enabled),
        "history_ring_chronological_outputs": True,
        "hist_len": int(adapter_env.hist_len),
