...
class CollisionReplayBuffer:
    ...
    def _slice_value(self, value: Any, *, env_id: int, batch_pos: int, batch_len: int, owned: bool = False) -> Any:
        if self._is_tensor(value):
            if len(value.shape) >= 1:
                first_dim = int(value.shape[0])
                if first_dim == self.num_envs:
                    sliced = value[env_id].detach()
                    return sliced if owned else sliced.clone()
                if first_dim == batch_len:
                    sliced = value[batch_pos].detach()
                    return sliced if owned else sliced.clone()
            detached = value.detach()
            return detached if owned else detached.clone()
        if isinstance(value, dict):
            return {
                key: self._slice_value(item, env_id=env_id, batch_pos=batch_pos, batch_len=batch_len, owned=owned)
        ...
        *,
        ...
        for batch_pos, env_id in enumerate(ids):
            ...
            record = {
                ...
                "root_state": self._slice_value(root_state, env_id=env_id, batch_pos=batch_pos, batch_len=batch_len, owned=owned),
                "dof_pos": self._slice_value(dof_pos, env_id=env_id, batch_pos=batch_pos, batch_len=batch_len, owned=owned),
                "dof_vel": self._slice_value(dof_vel, env_id=env_id, batch_pos=batch_pos, batch_len=batch_len, owned=owned),
                "command": self._slice_value(command, env_id=env_id, batch_pos=batch_pos, batch_len=batch_len, owned=owned),
                "sea_obs_hist": self._slice_value(sea_obs_hist, env_id=env_id, batch_pos=batch_pos, batch_len=batch_len, owned=owned),
                "slr_obs_hist": self._slice_value(slr_obs_hist, env_id=env_id, batch_pos=batch_pos, batch_len=batch_len, owned=owned),
                "task_state": self._slice_value(task_state, env_id=env_id, batch_pos=batch_pos, batch_len=batch_len, owned=owned),
